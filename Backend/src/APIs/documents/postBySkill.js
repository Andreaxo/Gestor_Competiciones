...
const { postBySkill } = require("../../Routes/documentsRoutes.js");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const skillId = req.params.idSkill;
      // body
      const { nombre, description } = req.body;

      //Verificar que no hayan campos vacios
      if (!nombre || !description || !skillId || !fileName) {
      ...
      await conexion.execute(
        "INSERT INTO documents (fileName,documentName, description, uploadDate, skillId) VALUES(?,?,?,?,?)",
        [fileName, nombre, description, uploadDate, skillId]
      );
