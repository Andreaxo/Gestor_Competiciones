...
const { patchSkill2 } = require("../../Routes/skillsRoutes.js"); // Ruta: /skills/patch2/:idSkill
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const updateQuery = idCompetidor
        ? "UPDATE skills SET comp2 = ? WHERE idSkills = ?"
        : "UPDATE skills SET comp2 = NULL WHERE idSkills = ?";
  ...
  : await mysqlDB.conexionDB.execute(updateQuery, [idSkill]);
      ...
      const mensaje = idCompetidor
        ? "Segundo competidor asignado correctamente"
        : "Segundo competidor eliminado correctamente";
